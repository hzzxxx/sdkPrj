package com.dating.sdklib.util.statubar;
...
public class StatusBarUtil {
     ...
     * 初始化沉浸状态栏
     */
    public static void initTransparentBar(Activity a) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT && Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) { // Android4.4 < = SDK版本 < Android5.0
            a.getWindow().addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS); //状态栏栏透明
            // a.getWindow().addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_NAVIGATION); //导航栏透明
            a.getWindow().clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS); //强制状态栏透明
            ...
            a.getWindow().setStatusBarColor(Color.TRANSPARENT); //设置StatusBar颜色为透明
        }
    }
}
